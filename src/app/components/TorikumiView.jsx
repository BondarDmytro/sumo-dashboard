  ...
  const renderMatch = (m) => {
    ...
    return (
      <div key={m.id} id={m.id === liveMatchId ? "tk-live-row" : undefined} className={"tk-match" + (m.id === liveMatchId ? " tk-live" : "") + ((isFav(m.eastId) || isFav(m.westId)) ? " fav-row" : "")} style={{
        ...
        {/* West */}
        <div style={{display:'grid',gridTemplateColumns: ((hasResult || wMark) ? (isMobile ? '12px auto minmax(0,1fr)' : (lang === 'ja' ? '14px 56px 22px minmax(0,1fr) 40px' : '14px 42px 22px minmax(0,1fr) 40px')) : (isMobile ? 'auto auto minmax(0,1fr)' : (lang === 'ja' ? '56px 22px minmax(0,1fr) 40px' : '42px 22px minmax(0,1fr) 40px')))  /* tk_cols_v4 */,gap:4,alignItems:'center',minWidth:0,opacity: hasResult && !westWon ? 0.4 : 1,cursor: pickable ? 'pointer' : 'default'}} onClick={pickable ? () => togglePick(m.matchNo, m.westId) : undefined}>{/* tk_cols_v2 + pickem_panel_v1 */}
          {(hasResult || wMark) && (
            wMark
