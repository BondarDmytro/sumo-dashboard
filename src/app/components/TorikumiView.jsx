...
import eloData from '../lib/eloRatings.json' /* tk_mob_stack_v1 */
  ...
  const renderMatch = (m) => {
    ...
    return (
      <div key={m.id} id={m.id === liveMatchId ? "tk-live-row" : undefined} className={"tk-match" + (m.id === liveMatchId ? " tk-live" : "") + ((isFav(m.eastId) || isFav(m.westId)) ? " fav-row" : "")} style={{
        ...
        {/* East */}
        <div style={{display:'grid',gridTemplateColumns: ((hasResult || eMark) ? (isMobile ? 'minmax(0,1fr) auto 12px' : (lang === 'ja' ? '56px minmax(0,1fr) 26px 22px 40px 14px' : '42px minmax(0,1fr) 26px 22px 40px 14px')) : (isMobile ? 'minmax(0,1fr) auto' : (lang === 'ja' ? '56px minmax(0,1fr) 26px 22px 40px' : '42px minmax(0,1fr) 26px 22px 40px')))  /* tk_cols_v4 */,gap:4,alignItems:'center',minWidth:0,opacity: hasResult && !eastWon ? 0.4 : 1,cursor: pickable ? 'pointer' : 'default'}} /* pickem_name_frame_v1 */ onClick={pickable ? () => togglePick(m.matchNo, m.eastId) : undefined}>{/* tk_cols_v2 + pickem_panel_v1 + tk_score_col_v1 */}
          {!isMobile && <span style={{display:'flex',gap:3,alignItems:'center',justifyContent:'center',minWidth:0}}> {/* tk_ovr_wrap_v1 */}<span style={{fontFamily:'monospace',fontSize: isMobile ? '0.52rem' : '0.56rem',color:rankColor(m.eastRank),fontWeight:700,background:rankColor(m.eastRank)+'2e',padding:'1px 4px',borderRadius:2,whiteSpace:'nowrap',textAlign:'center',width:'fit-content',margin:'0 auto'}}  /* tk_pill_fit_v2 */>{shortRank(m.eastRank, lang)}</span></span>}{!isMobile && <span style={{textAlign:'center'}}><OvrBadge id={m.eastId} /></span>}
          {!isMobile && <span style={{fontSize: isMobile ? '0.7rem' : '0.85rem',textAlign:'center'}}>{eastFlag}</span>}{/* tk_flag_outer_v1 tk_flag_meta_v1 */}<span style={{display:'flex',flexDirection:'column',alignItems:'center',minWidth:0,width:'100%',...pickStyle(m.eastId)}}><span style={{fontWeight: eastWon ? 800 : 600,fontSize: isMobile ? '0.72rem' : '0.88rem',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:'100%'}}>{lang === 'ja' ? String(eastR?.nameJp || m.eastJp || m.eastShikona).split('\u3000')[0] : m.eastShikona}</span>{isMobile && <span style={{display:'flex',gap:3,alignItems:'center',marginTop:1}}><span style={{fontSize:'0.55rem'}}>{eastFlag}</span><span style={{fontFamily:'monospace',fontSize:'0.44rem',fontWeight:700,color:rankColor(m.eastRank),background:rankColor(m.eastRank)+'2e',padding:'0px 3px',borderRadius:2,whiteSpace:'nowrap'}}>{shortRank(m.eastRank, lang)}</span><OvrBadge id={m.eastId} /></span>}</span>{/* tk_cols_v4: ja - lyshe shikona */}{/* tk_mob_badge_col_v1: beidzh vynos u kolonku */}  {/* tk_name_center_v1 */}
        ...
        {/* West */}
        <div style={{display:'grid',gridTemplateColumns: ((hasResult || wMark) ? (isMobile ? '12px auto minmax(0,1fr)' : (lang === 'ja' ? '14px 56px 22px 26px minmax(0,1fr) 40px' : '14px 42px 22px 26px minmax(0,1fr) 40px')) : (isMobile ? 'auto minmax(0,1fr)' : (lang === 'ja' ? '56px 22px 26px minmax(0,1fr) 40px' : '42px 22px 26px minmax(0,1fr) 40px')))  /* tk_cols_v4 */,gap:4,alignItems:'center',minWidth:0,opacity: hasResult && !westWon ? 0.4 : 1,cursor: pickable ? 'pointer' : 'default'}} onClick={pickable ? () => togglePick(m.matchNo, m.westId) : undefined}>{/* tk_cols_v2 + pickem_panel_v1 */}
          {(hasResult || wMark) && (
            wMark
          ...
          <span style={{display:'flex',flexDirection:'column',alignItems:'center',minWidth:0,width:'100%',...pickStyle(m.westId)}}><span style={{fontWeight: westWon ? 800 : 600,fontSize: isMobile ? '0.72rem' : '0.88rem',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:'100%'}}>{lang === 'ja' ? String(westR?.nameJp || m.westJp || m.westShikona).split('\u3000')[0] : m.westShikona}</span>{isMobile && <span style={{display:'flex',gap:3,alignItems:'center',marginTop:1}}><span style={{fontSize:'0.55rem'}}>{westFlag}</span><span style={{fontFamily:'monospace',fontSize:'0.44rem',fontWeight:700,color:rankColor(m.westRank),background:rankColor(m.westRank)+'2e',padding:'0px 3px',borderRadius:2,whiteSpace:'nowrap'}}>{shortRank(m.westRank, lang)}</span><OvrBadge id={m.westId} /></span>}</span>{!isMobile && <span style={{fontSize: isMobile ? '0.7rem' : '0.85rem',textAlign:'center'}}>{westFlag}</span>}{/* tk_cols_v4 */}{/* tk_mob_badge_col_v1 */}  {/* tk_name_center_v1 */}
          {!isMobile && <span style={{display:'flex',gap:3,alignItems:'center',justifyContent:'center',minWidth:0}}><span style={{fontFamily:'monospace',fontSize: isMobile ? '0.52rem' : '0.56rem',color:rankColor(m.westRank),fontWeight:700,background:rankColor(m.westRank)+'2e',padding:'1px 4px',borderRadius:2,whiteSpace:'nowrap',textAlign:'center',width:'fit-content',margin:'0 auto'}}>{shortRank(m.westRank, lang)}</span></span>}{!isMobile && <span style={{textAlign:'center'}}><OvrBadge id={m.westId} /></span>}
        </div>
      </div>
    )
  }
...
/* tk_mob_stack_v2 part1: meta pills */
