...
export default function TournamentStatus({ leaders, chasers, currentDay, maxWins, kyujoCount, contendersCount, eliminatedCount = 0, isFinished, topKimarite = null, winner = null, playoff = null }) {  /* ts_kimarite_v1 */  /* ts_eliminated_v1 */
    ...
    return (
      <>
        <div className="anim-1" style={{fontFamily:'monospace',fontSize:'0.72rem',letterSpacing:'0.2em',textTransform:'uppercase',color:'var(--mid)',borderBottom:'1px solid var(--border)',paddingBottom:'0.5rem',marginBottom:'0.8rem',marginTop:'1.2rem'}}>
          {t3(lang, 'Підсумки турніру', 'Tournament results', '\u5834\u6240\u7D50\u679C', 'Bilan du tournoi')}
        </div>
        <div className="anim-1" style={{display:'flex',gap:'1rem',alignItems:'stretch',background:'var(--card)',border:'1px solid #b8860b',borderRadius:4,padding:'0.9rem 1rem',marginBottom:'1.2rem',flexWrap:'wrap'}}>
          ...
          <div style={{flex:1,minWidth:200,display:'flex',flexDirection:'column',justifyContent:'center',gap:4}}>
            <div style={{fontFamily:'monospace',fontSize:'0.58rem',letterSpacing:'0.14em',textTransform:'uppercase',color:'#b8860b'}}>{String.fromCodePoint(0x1F3C6)} {t3(lang, 'Юшо', 'Yusho champion', '\u512A\u52DD', 'Champion yusho')}</div>
            ...
            {playoff?.bouts?.length > 0 && (
              <div style={{marginTop:6,paddingTop:6,borderTop:'1px solid var(--border)'}}>
                <div style={{fontFamily:'monospace',fontSize:'0.52rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'var(--mid)',marginBottom:3}}>{t3(lang, 'Плей-оф', 'Playoff', '\u512A\u52DD\u6C7A\u5B9A\u6226', 'Barrage')}</div>
  ...
  const rec = t3(lang, 'рекорд', 'record', '成績', 'bilan')
  const stats = [
    {
      num: leaders.length,
      label: t3(lang, 'Лідери', 'Leaders', 'トップ', 'Leaders'),
      sub: `${rec} ${maxWins}–${leaders[0]?.losses ?? '?'}`,
      color: '#b8860b'
    },
    {
      num: chasers.length,
      label: t3(lang, 'Переслідувачі', 'Chasers', '追う力士', 'Poursuivants'),
      sub: `${rec} ${maxWins-1}–${chasers[0]?.losses ?? '?'}`,
      color: '#1a6b5c'
    },
    {
      num: contendersCount,
      label: t3(lang, 'Претендентів', 'Contenders', '優勝候補', 'Prétendants'),
      sub: t3(lang, 'шанс > 0%', 'chance > 0%', '確率 > 0%', 'chance > 0%'),
      color: 'var(--mid)',
    },
    ...(eliminatedCount > 0 ? [{
      num: eliminatedCount,
      label: t3(lang, 'Вибули', 'Eliminated', '脱落', 'Éliminés'),
      sub: t3(lang, 'шанс 0%', 'chance 0%', '確率 0%', 'chance 0%'),
      color: 'var(--ink)'
    }] : []),
    ...(topKimarite ? [{
      num: topKimarite.name,
      label: t3(lang, 'Техніка турніру', 'Top kimarite', '決まり手', 'Top kimarite'),
      sub: `${topKimarite.count} \u00b7 ${topKimarite.pct}%`,
      color: '#7d3c98'
    }] : []),  /* ts_kimarite_v2 */
    ...(15 - currentDay > 0 ? [{
      num: 15 - currentDay,
      label: t3(lang, 'Днів залишилось', 'Days remaining', '残り日数', 'Jours restants'),
      sub: t3(lang, 'до фіналу', 'to final', '千秋楽まで', 'avant la finale'),
      color: 'var(--ink)'
    }] : []),
    {
      num: kyujoCount,
      label: t3(lang, 'Кюджо', 'Kyujo', '休場', 'Kyujo'),
      sub: t3(lang, 'відсутні', 'absent', '不在', 'absents'),
      color: '#c0392b'
    },
  ]
  ...
  return (
    <>
      <div className="anim-1" style={{fontFamily:'monospace',fontSize:'0.72rem',letterSpacing:'0.2em',textTransform:'uppercase',color:'var(--mid)',borderBottom:'1px solid var(--border)',paddingBottom:'0.5rem',marginBottom:'0.8rem',marginTop:'1.2rem'}}>
        {t3(lang, 'Стан турніру', 'Tournament status', '場所の状況', 'État du tournoi')}
      </div>
