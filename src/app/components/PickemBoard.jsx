...
export default function PickemBoard({ bashoId, currentDay, myUid, open, onClose }) {
  ...
  const guest = (uid) => '\u0413\u0456\u0441\u0442\u044c-' + String(uid).replace(/-/g, '').slice(-4).toUpperCase()
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.55)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div onClick={e => e.stopPropagation()} style={{background:'var(--card)',border:'1px solid rgba(184,134,11,0.4)',borderRadius:6,width:'min(440px, 100%)',maxHeight:'80vh',display:'flex',flexDirection:'column',boxShadow:'0 16px 48px rgba(0,0,0,0.4)'}}>
        <div style={{display:'flex',alignItems:'center',padding:'12px 16px',borderBottom:'1px solid var(--border)'}}>
          ...
          <button onClick={onClose} style={{background:'transparent',border:'none',cursor:'pointer',fontSize:18,color:'var(--mid)',padding:4}}>{'\u2715'}</button>
        </div>
            ...
            <>
              {shown.map((r, i) => (
                <div key={r.uid} style={{display:'flex',alignItems:'center',gap:8,padding:'5px 6px',borderBottom:'1px solid var(--border)',background: r.me ? 'rgba(184,134,11,0.12)' : 'transparent',borderRadius:2}}>
                  ...
                  <span style={{flex:1,fontWeight: r.me ? 800 : 500,color:'var(--ink)'}}>{r.me ? '\u2605 ' + t3(lang, 'ти', 'you', 'あなた') : guest(r.uid)}</span>
              ...
              {myIdx >= 10 && (
                <div style={{display:'flex',alignItems:'center',gap:8,padding:'5px 6px',marginTop:6,background:'rgba(184,134,11,0.12)',borderRadius:2}}>
                  ...
                  <span style={{flex:1,fontWeight:800,color:'var(--ink)'}}>{'\u2605 '}{t3(lang, 'ти', 'you', 'あなた')}</span>
