  ...
  return (
    <div className="cd-outer" style={{ marginTop: '0.9rem' }}>
      <div style={{ fontFamily: 'monospace', fontSize: '0.8rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#b8c7c8', marginBottom: 8 }}>
        {t3(lang, `До старту — ${bashoLabel}`, `Countdown — ${bashoLabel}`, `開始まで — ${bashoLabel}`, `Compte à rebours — ${bashoLabel}`)}
      </div>
      <div className="cd-wrap" style={{ display: 'flex', gap: 'clamp(4px, 1.6vw, 10px)', alignItems: 'center', flexWrap: 'nowrap' /* countdown_fluid_v1 */ }}>
        <div style={cell}><div style={num}>{pad(d)}</div><div style={lab}>{t3(lang,'днів','days','日', 'jours')}</div></div>
        <div style={{ ...num, opacity: 0.4 }}>:</div>
        <div style={cell}><div style={num}>{pad(h)}</div><div style={lab}>{t3(lang,'год','hrs','時間', 'h')}</div></div>
        <div style={{ ...num, opacity: 0.4 }}>:</div>
        <div style={cell}><div style={num}>{pad(m)}</div><div style={lab}>{t3(lang,'хв','min','分', 'min')}</div></div>
        <div style={{ ...num, opacity: 0.4 }}>:</div>
        <div style={cell}><div style={num}>{pad(s)}</div><div style={lab}>{t3(lang,'сек','sec','秒', 'sec')}</div></div>
      </div>
      <div style={{ fontFamily: 'monospace', fontSize: '0.8rem', color: '#6b6560', marginTop: 10 }}>
        <div>🇯🇵 {jst} JST</div>  {/* jst_two_lines */}
        <div style={{marginTop: 2}}>🕐 {t3(lang, 'у вас', 'your time', '現地', 'chez vous')}: {local}</div>
      </div>
    </div>
  )
}
