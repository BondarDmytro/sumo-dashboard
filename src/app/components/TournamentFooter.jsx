...
export default function TournamentFooter({ contenders, h2h, allRikishi = null }) {
  ...
  const topAtDay = (allRikishi && allRikishi.length)
    ? [...allRikishi].filter(r => !r.kyujo).sort((a, b) => ((b.yushoChance ?? b.chancePct ?? 0) - (a.yushoChance ?? a.chancePct ?? 0))  /* h2h_top_by_chance_v2: realne pole dvizhka */ || (winsAt(b, day) - winsAt(a, day))).slice(0, 10)  /* h2h_top_by_chance_v1: toi samyi kryterii, shcho tablytsia */
  ...
  return (
    <>
      <div className="anim-4" style={{fontFamily:'monospace',fontSize:'0.72rem',letterSpacing:'0.2em',textTransform:'uppercase',color:'var(--mid)',borderBottom:'1px solid var(--border)',paddingBottom:'0.5rem',marginBottom:'1.2rem',marginTop:'2rem'}}>
        {t3(lang, 'Графік ймовірностей юшо', 'Yusho probability chart', '優勝確率チャート', 'Probabilités de yusho')}
      </div>
      ...
      <div style={{display:'flex',gap:4,flexWrap:'wrap',marginTop:'0.5rem',marginBottom:'1rem',alignItems:'center'}}>  {/* day_switch_v1: spilnyi peremykach dniv */}
        <span style={{fontFamily:'monospace',fontSize:'0.56rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--mid)',marginRight:6}}>{t3(lang, 'День', 'Day', String.fromCharCode(0x65E5), 'Jour')}</span>
      ...
      <div className="anim-5" style={{fontFamily:'monospace',fontSize:'0.72rem',letterSpacing:'0.2em',textTransform:'uppercase',color:'var(--mid)',borderBottom:'1px solid var(--border)',paddingBottom:'0.5rem',marginBottom:'1.2rem',marginTop:'2rem'}}>
        {t3(lang, 'Очні зустрічі — цей турнір (топ претенденти)', 'Head-to-head — this tournament (top contenders)', '対戦成績 — 今場所（上位陣）', 'Face-à-face — ce tournoi (top prétendants)')}
      </div>
      ...
      <div className="anim-6" style={{marginTop:'2.5rem',paddingTop:'1.5rem',borderTop:'1px solid var(--border)',fontSize:'0.72rem',color:'var(--mid)',lineHeight:1.7}}>
        ...
        ) : lang === 'fr' ? ( /* fr_footer_full_v1 */
          <><b style={{color:'var(--ink)'}}>Données :</b> sumo-api.com · mise à jour toutes les 60 secondes · <b style={{color:'var(--ink)'}}>Méthodologie :</b> modèle logistique entraîné sur 36 basho (2020–2026) : bilan, écart au leader, rang, forme sur 5 jours. Précision top-1 au jour 10 ≈ 50–60 % (holdout). Fusen (✦) — victoire par forfait de l’adversaire. Site de fans statistique gratuit.</>
        ) : lang === 'en' ? ( /* fr_footer_method_v1 */
