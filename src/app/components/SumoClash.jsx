...
const SALT_CARDS = [
  { id:'Sa1', type:'salt', label:'Сіль в обличчя', labelEn:'Salt Throw', effect:'⏩ Пропуск', color:'#7f8c8d', emoji:'🧂' },
]
const HENKA_CARDS = [
  { id:'He1', type:'henka', label:'Хенка', labelEn:'Henka', effect:'🌀 Ухилення', color:'#8e44ad', emoji:'🌀' },
]
...
function playSound(ctx,type){
  ...
  try{
    ...
    if(type==='clash'){
      ...
      // Бойовий вигук — низький гучний звук
      const o=ctx.createOscillator();const g=ctx.createGain();o.connect(g);g.connect(ctx.destination)
      o.type='sawtooth';o.frequency.setValueAtTime(180,now);o.frequency.exponentialRampToValueAtTime(120,now+0.2)
      g.gain.setValueAtTime(0,now);g.gain.linearRampToValueAtTime(0.4,now+0.05);g.gain.exponentialRampToValueAtTime(0.001,now+0.35)
      o.start(now);o.stop(now+0.35)
      // Відлуння
      const o2=ctx.createOscillator();const g2=ctx.createGain();o2.connect(g2);g2.connect(ctx.destination)
      o2.type='sine';o2.frequency.value=240;g2.gain.setValueAtTime(0.15,now+0.1);g2.gain.exponentialRampToValueAtTime(0.001,now+0.4);o2.start(now+0.1);o2.stop(now+0.4)
    } else if(type==='armor'){
      // Металевий удар
      [500,750,1000,1400].forEach((freq,i)=>{
        const o=ctx.createOscillator();const g=ctx.createGain();o.connect(g);g.connect(ctx.destination)
        o.type='square';o.frequency.value=freq
        g.gain.setValueAtTime(0.12,now+i*0.02);g.gain.exponentialRampToValueAtTime(0.001,now+i*0.02+0.2)
        o.start(now+i*0.02);o.stop(now+i*0.02+0.2)
      })
      // Металевий відзвук
      ...
      const g=ctx.createGain();g.gain.value=0.2;s.connect(f);f.connect(g);g.connect(ctx.destination);s.start(now)
    } else if(type==='strike'){
      // Блискавка — різкий електричний тріск
      ...
      [523,659,784,1047].forEach((freq,i)=>{
        ...
        g.gain.setValueAtTime(0,now+i*0.12);g.gain.linearRampToValueAtTime(0.3,now+i*0.12+0.05);g.gain.exponentialRampToValueAtTime(0.001,now+i*0.12+0.4)
      ...
      [400,350,300].forEach((freq,i)=>{
        ...
        g.gain.setValueAtTime(0.25,now+i*0.18);g.gain.exponentialRampToValueAtTime(0.001,now+i*0.18+0.35)
  ...
  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:4,gap:8}}>
        ...
        {armor>0&&<div style={{display:'flex',alignItems:'center',gap:4,background:'rgba(31,97,141,0.15)',border:'1px solid rgba(31,97,141,0.4)',borderRadius:3,padding:'1px 7px'}}><span style={{fontSize:'0.7rem'}}>🛡</span><span style={{fontFamily:'monospace',fontSize:'0.75rem',fontWeight:700,color:'#1f618d'}}>{armor}</span></div>}
      </div>
...
function AudioControls({sfxOn,musicOn,currentTheme,onToggleSfx,onToggleMusic,onThemeChange,lang}){
  ...
  return(
    <div style={{position:'relative',display:'flex',gap:6,alignItems:'center'}}>
      ...
      <button onClick={()=>setShowThemes(v=>!v)} style={{background:'var(--bg2)',border:'1px solid var(--border)',color:'var(--mid)',borderRadius:4,padding:'3px 8px',cursor:'pointer',fontFamily:'monospace',fontSize:'0.6rem'}}>
        🎼
      </button>
      {showThemes&&(
        <div onClick={e=>e.stopPropagation()} style={{position:'absolute',top:'100%',right:0,marginTop:4,background:'var(--card)',border:'1px solid var(--border)',borderRadius:4,padding:'0.5rem',zIndex:10,minWidth:160,boxShadow:'0 4px 12px rgba(0,0,0,0.3)'}}>
          ...
          {MUSIC_THEMES.map(th=>(
            <div key={th.id} onClick={()=>{onThemeChange(th.id);setShowThemes(false)}} style={{
              display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,
              padding:'4px 6px',borderRadius:3,cursor:'pointer',marginBottom:2,
              background:currentTheme===th.id?'rgba(184,134,11,0.2)':'transparent',
              border:`1px solid ${currentTheme===th.id?'#b8860b':'transparent'}`,
            }}>
              <div>
...
function RoundResult({myCard,oppCard,roundLog,myLabel,oppLabel,onNext,roundNum,lang}){
  ...
  useEffect(()=>{
    const t1=setTimeout(()=>setStep(1),100);const t2=setTimeout(()=>setStep(2),700);const t3=setTimeout(()=>setStep(3),1000)
  ...
  return(
    <div style={{animation:'slideIn 0.25s ease'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:'1rem'}}>
        <div style={{background:'var(--bg2)',padding:'0.75rem',borderRadius:4,border:'1px solid rgba(26,107,92,0.3)'}}><div style={{fontFamily:'monospace',fontSize:'0.65rem',color:'#1a6b5c',marginBottom:6,textTransform:'uppercase',fontWeight:700}}>{myLabel}</div><HPBar hp={myHp} armor={myArmor}/></div>
        <div style={{background:'var(--bg2)',padding:'0.75rem',borderRadius:4,border:'1px solid rgba(192,57,43,0.3)'}}><div style={{fontFamily:'monospace',fontSize:'0.65rem',color:'#c0392b',marginBottom:6,textTransform:'uppercase',fontWeight:700}}>{oppLabel}</div><HPBar hp={oppHp} armor={oppArmor}/></div>
      </div>
      {isRoundResult?(
...
function CpuGame({lang,onBack,sfx}){
  ...
  function nextRound(){const nr=roundNum+1;setRoundNum(nr);setPlayerSelected(null);setRoundLog([]);if(nr>=MAX_ROUNDS||playerHand.length===0)setPhase('gameOver');else setPhase('battle')}
...
export default function SumoClash({onClose,lang='uk'}){
  ...
  function ensureCtx(){
    ...
    if(!musicGainRef.current&&audioCtxRef.current){const g=audioCtxRef.current.createGain();g.gain.value=0.2;g.connect(audioCtxRef.current.destination);musicGainRef.current=g}
