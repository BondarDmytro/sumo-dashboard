    ...
    <div style={{animation:doShake&&step>=2?'shake 0.4s ease':undefined}}>
      ...
      <div style={{position:'relative',borderRadius:8,overflow:'hidden',marginBottom:'1.25rem'}}>
        ...
        <div style={{position:'relative',zIndex:1,display:'flex',justifyContent:'space-around',alignItems:'center',gap:16,padding:'1.2rem 0.5rem',opacity:step>=1?1:0,transition:'opacity 0.3s'}}>
          ...
          <div style={{fontFamily:'Georgia,serif',fontSize:'1.5rem',fontWeight:800,color:'#f0c060',textShadow:'0 0 20px rgba(240,192,96,0.8), 0 2px 6px rgba(0,0,0,0.9)',animation:step>=1?'pop 0.4s ease 0.4s both':'none'}}>VS</div>
          <div style={{textAlign:'center',position:'relative'}}>
            <div style={{fontFamily:'monospace',fontSize:'0.65rem',color:'rgba(255,180,150,0.9)',marginBottom:8,textShadow:'0 1px 4px rgba(0,0,0,0.8)'}}>{oppLabel}</div>
...
function MultiGame({lang,onBack,sfx}){
  ...
  async function doResolveRound(data,sid){
    ...
    await update(ref(db),{
      [`clash/${sid}/chat/${sysTs}`]:{text:sysMsg,role:'system',ts:sysTs},
      [`clash/${sid}/chat/${sysTs+1}`]:{text:winMsg,role:'system',ts:sysTs+1},
    })
  }
          ...
          <div style={{height:120,overflowY:'auto',padding:'6px 10px',display:'flex',flexDirection:'column',gap:4}}>
            ...
            {chatMessages.map(m=>(
              <div key={m.ts} style={{display:'flex',gap:6,alignItems:'flex-start'}}>
                {m.role==='system'?(
                  <span style={{fontFamily:'monospace',fontSize:'0.6rem',color:'#b8860b',lineHeight:1.5,fontStyle:'italic',opacity:0.85,width:'100%'}}>{m.text}</span>
                ):(
                  <>
