      ...
      {open && typeof document !== 'undefined' && createPortal(
        <div data-basho-popover="1" style={{ position: 'fixed', top: 110, right: 24, zIndex: 9999,  /* select_fix_v3 */ width: 340, maxHeight: 'calc(100vh - 140px)', overflowY: 'auto',  /* select_fix_v2 */
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            <button style={mode === 'year' ? btnActive : btn} onClick={() => setMode('year')}>{t3(lang, 'За роком', 'By year', '年別', 'Par année')}</button>
            <button style={mode === 'basho' ? btnActive : btn} onClick={() => setMode('basho')}>{t3(lang, 'За башьо', 'By basho', '場所別', 'Par basho')}</button>
          </div>
            ...
            <div>
              ...
              {year && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: 'repeat(3, auto)', gridAutoFlow: 'column', gap: 6, borderTop: '1px solid rgba(240,192,96,0.2)', paddingTop: 10 }}>  {/* basho_select_layout_v4: basho 2 kolonky po-kolonkakh */}
                  ...
                  {idsForYear(year).length === 0 && <span style={{ fontFamily: 'monospace', fontSize: '0.65rem', color: '#8a8378' }}>{t3(lang, 'Нема даних', 'No data', 'データなし', 'Pas de données')}</span>}
                </div>
              )}
            </div>
          )}
